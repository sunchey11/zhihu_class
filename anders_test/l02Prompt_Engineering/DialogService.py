...
# NLU（Natural Language Understanding,语义理解）
class NLU:
    def __init__(self):
...
class DialogManager:
    ...
    def run(self, user_input):
        # 调用NLU获得语义解析
        # 1.将语言描述，转换为程序可理解的json，json存放过滤规则和条件排序信息
        ...
        # 根据状态检索DB，获得满足条件的候选
        # 2.根据条件查询数据库，选中一条或多条数据（只选第一条）
        records = self.db.retrieve(**self.state)

        # 拼装prompt调用chatgpt，将选中的这条数据通过模板变成一段文字
        # 3.如：向用户介绍如下产品：经济套餐，月费50元，每月流量10G。
        prompt_for_chatgpt = self._wrap(user_input, records)
        print("===gpt-prompt===")
        print(prompt_for_chatgpt)

        # 调用chatgpt获得回复
        # 4.将找到的这条数据通过gpt美化一下，得到一段非常友好的文字
        response = self._call_chatgpt(prompt_for_chatgpt)

        # 将当前用户输入和系统回复维护入chatgpt的session
        # 这个session存储的是对话的历史，给上一步_call_chatgpt用的

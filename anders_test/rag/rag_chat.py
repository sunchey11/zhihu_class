...
def extract_text_from_pdf(filename, page_numbers=None, min_line_length=1):
    ...
    for i, page_layout in enumerate(extract_pages(filename)):
        # 我加的，没必要再循环了，很慢
        if page_numbers is not None and i > max(page_numbers):
            break
        # 如果指定了页码范围，跳过范围外的页
    ...
    return paragraphs
# 测试一下
paragraphs = extract_text_from_pdf(abs_path("llama2.pdf"),  page_numbers=[2, 3], 
    min_line_length=10)
...
# 结果为啥在results['documents'][0]，'documents'为啥是个数组

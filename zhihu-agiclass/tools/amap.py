...
def get_distance_time(origin, destination):
    ...
    """
    print(f'调用本地方法 get_distance_time({origin}, {destination})')
    # 获取起点和终点的经纬度和POI ID
    ...
    # return round(int(distance) / 1000, 2), round(int(time) / 60, 2)
    print(f'本地方法 get_distance_time 返回结果：{round(int(distance) / 1000, 2)}, {round(int(time) / 60, 2)}')
